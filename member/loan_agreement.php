...
// Check if user is logged in and is either approved member or admin
if (!isset($_SESSION['user']) || 
    ($_SESSION['user']['role'] !== 'member' && $_SESSION['user']['role'] !== 'admin') ||
    ($_SESSION['user']['role'] === 'member' && $_SESSION['user']['status'] !== 'active')) {
...
// Modify the loan verification query to allow admin access
$pdo = DB::pdo();
if ($user['role'] === 'admin') {
    // Admin can view any approved/active/completed loan
    $stmt = $pdo->prepare("
    ...
    $stmt = $pdo->prepare("
        ...
        FROM loans l 
        INNER JOIN users u ON l.user_id = u.member_id 
        WHERE l.id = ? AND l.user_id = ? AND l.status IN ('approved', 'active', 'completed')
    ");
    $stmt->execute([$loan_id, $user['member_id']]);
}
...
$address_parts = [];
if (!empty($loan['house_no'])) $address_parts[] = $loan['house_no'];
if (!empty($loan['street_village'])) $address_parts[] = $loan['street_village'];
if (!empty($loan['barangay'])) $address_parts[] = $loan['barangay'];
if (!empty($loan['municipality'])) $address_parts[] = $loan['municipality'];
if (!empty($loan['city'])) $address_parts[] = $loan['city'];
if (!empty($loan['postal_code'])) $address_parts[] = $loan['postal_code'];
    ...
    <div class="container">
        ...
        <div class="actions no-print">
            <?php if ($_SESSION['user']['role'] === 'admin'): ?>
                <a href="../admin/apply_member_loan.php?member_id=<?= $loan['member_id'] ?>" class="btn btn-secondary">
                    <i class="fas fa-arrow-left me-1"></i>Back to Member
                </a>
            <?php else: ?>
                <a href="loans.php?view_loan=<?= $loan_id ?>" class="btn btn-secondary">
                    <i class="fas fa-arrow-left me-1"></i>Back to Loan Details
                </a>
            <?php endif; ?>
            
            <a href="?action=print&loan_id=<?= $loan_id ?>" class="btn btn-primary" onclick="window.print(); return false;">
                <i class="fas fa-print me-1"></i>Print
            </a>
            
            <?php if ($_SESSION['user']['role'] === 'admin'): ?>
                <a href="../admin/loans.php?action=view&id=<?= $loan_id ?>" class="btn btn-info">
                    <i class="fas fa-eye me-1"></i>View in Loans
                </a>
            <?php endif; ?>
        </div>
        ...
        <div class="section">
            ...
            <div class="clause">
                ...
                <p><strong><?= $loan['firstname'] . ' ' . $loan['lastname'] ?></strong> ("Borrower"), with Member ID: <?= $loan['member_id'] ?>, residing at <?= $address ?>.</p>
            </div>
        </div>
